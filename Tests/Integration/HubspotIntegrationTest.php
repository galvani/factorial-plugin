...
use MauticPlugin\MauticFactorialBundle\Integration\HubspotIntegration;
...
class HubspotIntegrationTest extends AbstractIntegrationTestCase
{
    ...
    private FactorialHubspotIntegration $integration;
    ...
    protected function setUp(): void
    {
        ...
        $this->integration = new FactorialHubspotIntegration(
            $this->dispatcher,
            $this->cache,
            $this->em,
            $this->session,
    ...
    public function testGetBearerTokenEmpty(): void
    {
        ...
        $this->dispatcher->expects(self::once())
            ->method('dispatch')
            ->with(
                new PluginIntegrationKeyEvent($this->integration, [FactorialHubspotIntegration::ACCESS_KEY]),
                PluginEvents::PLUGIN_ON_INTEGRATION_KEYS_ENCRYPT
            )
            ->willReturn($event);

        $this->integration->encryptAndSetApiKeys([FactorialHubspotIntegration::ACCESS_KEY], $this->createMock(Integration::class));
    ...
    public function testGetBearerTokenSet(): void
    {
        ...
        $event->expects(self::once())
            ->method('getKeys')
            ->willReturn(['other' => 'data', FactorialHubspotIntegration::ACCESS_KEY => $token]);
        $this->dispatcher->expects(self::once())
            ->method('dispatch')
            ->with(
                new PluginIntegrationKeyEvent($this->integration, [FactorialHubspotIntegration::ACCESS_KEY]),
    ...
    public function testAppendToFormKeys(): void
    {
        ...
        $builder->expects(self::exactly(2))
            ->method('add')
            ->withConsecutive(
                [FactorialHubspotIntegration::ACCESS_KEY, TextType::class],
