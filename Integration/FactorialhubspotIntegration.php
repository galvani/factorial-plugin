...
use MauticPlugin\MauticFactorialBundle\Api\HubspotApi;
use Monolog\Logger;
...
class FactorialhubspotIntegration extends CrmAbstractIntegration
{
    ...
    {
        return 'Factorialhubspot';
    }

    public function getDisplayName(): string
    {
        return 'Hubspot Factorial';
    }

    public function getIcon(): string
    {
        return 'plugins/MauticFactorialBundle/Assets/img/hubspot.png';
    }

    public function getApiHelper(): HubspotApi
    {
        if (empty($this->helper)) {
            $this->helper = new HubspotApi($this);
        }
            ...
            if ($this->isAuthorized()) {
                ...
                foreach ($results['results'] as $company) {
                    if (isset($company['properties'])) {
                        $companyData = [];
                        foreach ($company['properties'] as $key => $field) {
    ...
    public function getMauticLead($data, $persist = true, $socialCache = null, $identifiers = null, $object = null)
    {
        ...
        if (isset($data['associatedcompanyid'])) {
            if (strlen(trim($data['associatedcompanyid']))) {
        ...
        if ($lead = parent::getMauticLead($data, false, $socialCache, $identifiers, $object)) {
            ...
            if ($persist && !empty($lead->getChanges(true))) {
                ...
                try {
                    ...
                    if (isset($company)) {
                        // we need to rerun point model so the companie's properties propagate
